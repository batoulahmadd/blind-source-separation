...
T   = [3, 4, 5, 6, 7];        % periods for each signal
d   = 5;                % mixed observations
r   = 5;                % independent/principal components
...
Ztrue(2,:)  = sin(t(numBits,T(2)));
Ztrue(3,:) = sin(t(numBits,T(3)));
Ztrue(4,:) = sin(t(numBits,T(4)));
Ztrue(5,:) = sin(t(numBits,T(5)));
    ...
    if numComponentsFastICA >= 4
        corrfastICA(4, i) = corr((Ztrue(1,:))', (datafastICA(4,:))');    
    end
    if numComponentsFastICA >= 5
        corrfastICA(5, i) = corr((Ztrue(1,:))', (datafastICA(5,:))');    
    end
    ...
    if numComponentsAMUSE >= 3
        ...
        if numComponentsAMUSE >= 4
        corrAMUSE(4, i) = corr((Ztrue(1,:))',( dataAMUSE(4,:))');
    end
    if numComponentsAMUSE >= 5
        corrAMUSE(5, i) = corr((Ztrue(1,:))',( dataAMUSE(5,:))');
    end
    ...
    if numComponentsSOBI >= 4
        corrSOBI(4, i) = corr((Ztrue(1,:))', (dataSOBI(4,:))');
    end
    if numComponentsSOBI >= 5
        corrSOBI(5, i) = corr((Ztrue(1,:))', (dataSOBI(5,:))');
    end
    best_corSOBI(1, i) = max(abs(corrSOBI(:, i)));

    
    % For the second Sinusoid signal
    % Reset the correlation coefficients 
    corrfastICA=zeros(r,numSimulations);
    corrAMUSE=zeros(r,numSimulations);
    corrSOBI=zeros(r,numSimulations);
    ...
    if numComponentsFastICA >= 4
        corrfastICA(4, i) = corr((Ztrue(2,:))', (datafastICA(4,:))');    
    end
    if numComponentsFastICA >= 5
        corrfastICA(5, i) = corr((Ztrue(2,:))', (datafastICA(5,:))');    
    end
    ...
    if numComponentsAMUSE >= 3
        ...
        if numComponentsAMUSE >= 4
        corrAMUSE(4, i) = corr((Ztrue(2,:))',( dataAMUSE(4,:))');
    end
    if numComponentsAMUSE >= 5
        corrAMUSE(5, i) = corr((Ztrue(2,:))',( dataAMUSE(5,:))');
    end
    ...
    if numComponentsSOBI >= 4
        corrSOBI(4, i) = corr((Ztrue(2,:))', (dataSOBI(4,:))');
    end
    if numComponentsSOBI >= 5
        corrSOBI(5, i) = corr((Ztrue(2,:))', (dataSOBI(5,:))');
    end
    best_corSOBI(2, i) = max(abs(corrSOBI(:, i)));
     
    
    % For the third Sinusoid signal
    % Reset the correlation coefficients 
    corrfastICA=zeros(r,numSimulations);
    corrAMUSE=zeros(r,numSimulations);
    corrSOBI=zeros(r,numSimulations);
    ...
    if numComponentsFastICA >= 4
        corrfastICA(4, i) = corr((Ztrue(3,:))', (datafastICA(4,:))');    
    end
    if numComponentsFastICA >= 5
        corrfastICA(5, i) = corr((Ztrue(3,:))', (datafastICA(5,:))');    
    end
    ...
    if numComponentsAMUSE >= 3
        ...
        if numComponentsAMUSE >= 4
        corrAMUSE(4, i) = corr((Ztrue(3,:))',( dataAMUSE(4,:))');
    end
    if numComponentsAMUSE >= 5
        corrAMUSE(5, i) = corr((Ztrue(3,:))',( dataAMUSE(5,:))');
    end
    ...
    if numComponentsSOBI >= 4
        corrSOBI(4, i) = corr((Ztrue(3,:))', (dataSOBI(4,:))');
    end
    if numComponentsSOBI >= 5
        corrSOBI(5, i) = corr((Ztrue(3,:))', (dataSOBI(5,:))');
    end
    best_corSOBI(3, i) = max(abs(corrSOBI(:, i)));
    
    
    % For the fourth Sinusoid signal
    % Reset the correlation coefficients 
    corrfastICA=zeros(r,numSimulations);
    corrAMUSE=zeros(r,numSimulations);
    corrSOBI=zeros(r,numSimulations);
    %compute the correlation between the original and the estimated signals
     corrfastICA(1, i) = corr((Ztrue(4,:))', (datafastICA(1,:))');
    if numComponentsFastICA >= 2
        corrfastICA(2, i) = corr((Ztrue(4,:))', (datafastICA(2,:))');   
    end
    if numComponentsFastICA >= 3
        corrfastICA(3, i) = corr((Ztrue(4,:))', (datafastICA(3,:))');    
    end
    if numComponentsFastICA >= 4
        corrfastICA(4, i) = corr((Ztrue(4,:))', (datafastICA(4,:))');    
    end
    if numComponentsFastICA >= 5
        corrfastICA(5, i) = corr((Ztrue(4,:))', (datafastICA(5,:))');    
    end
    best_corfastICA(4, i) = max(abs(corrfastICA(:, i)));
    
    corrAMUSE(1, i) = corr((Ztrue(4,:))',( dataAMUSE(1,:))');
    if numComponentsAMUSE >= 2
        corrAMUSE(2, i) = corr((Ztrue(4,:))',( dataAMUSE(2,:))');
    end
    if numComponentsAMUSE >= 3
        corrAMUSE(3, i) = corr((Ztrue(4,:))',( dataAMUSE(3,:))');
    end
        if numComponentsAMUSE >= 4
        corrAMUSE(4, i) = corr((Ztrue(4,:))',( dataAMUSE(4,:))');
    end
    if numComponentsAMUSE >= 5
        corrAMUSE(5, i) = corr((Ztrue(4,:))',( dataAMUSE(5,:))');
    end
    best_corAMUSE(4, i) = max(abs(corrAMUSE(:, i)));
    
    corrSOBI(1, i) = corr((Ztrue(4,:)'),( dataSOBI(1,:))');
    if numComponentsSOBI >= 2
        corrSOBI(2, i) = corr((Ztrue(4,:))', (dataSOBI(2,:))');
    end
    if numComponentsSOBI >= 3
        corrSOBI(3, i) = corr((Ztrue(4,:))', (dataSOBI(3,:))');
    end
    if numComponentsSOBI >= 4
        corrSOBI(4, i) = corr((Ztrue(4,:))', (dataSOBI(4,:))');
    end
    if numComponentsSOBI >= 5
        corrSOBI(5, i) = corr((Ztrue(4,:))', (dataSOBI(5,:))');
    end
    best_corSOBI(4, i) = max(abs(corrSOBI(:, i)));
    
    
    % For the fifth Sinusoid signal
    % Reset the correlation coefficients 
    corrfastICA=zeros(r,numSimulations);
    corrAMUSE=zeros(r,numSimulations);
    corrSOBI=zeros(r,numSimulations);
    %compute the correlation between the original and the estimated signals
     corrfastICA(1, i) = corr((Ztrue(5,:))', (datafastICA(1,:))');
    if numComponentsFastICA >= 2
        corrfastICA(2, i) = corr((Ztrue(5,:))', (datafastICA(2,:))');   
    end
    if numComponentsFastICA >= 3
        corrfastICA(3, i) = corr((Ztrue(5,:))', (datafastICA(3,:))');    
    end
    if numComponentsFastICA >= 4
        corrfastICA(4, i) = corr((Ztrue(5,:))', (datafastICA(4,:))');    
    end
    if numComponentsFastICA >= 5
        corrfastICA(5, i) = corr((Ztrue(5,:))', (datafastICA(5,:))');    
    end
    best_corfastICA(5, i) = max(abs(corrfastICA(:, i)));
    
    corrAMUSE(1, i) = corr((Ztrue(5,:))',( dataAMUSE(1,:))');
    if numComponentsAMUSE >= 2
        corrAMUSE(2, i) = corr((Ztrue(5,:))',( dataAMUSE(2,:))');
    end
    if numComponentsAMUSE >= 3
        corrAMUSE(3, i) = corr((Ztrue(5,:))',( dataAMUSE(3,:))');
    end
        if numComponentsAMUSE >= 4
        corrAMUSE(4, i) = corr((Ztrue(5,:))',( dataAMUSE(4,:))');
    end
    if numComponentsAMUSE >= 5
        corrAMUSE(5, i) = corr((Ztrue(5,:))',( dataAMUSE(5,:))');
    end
    best_corAMUSE(5, i) = max(abs(corrAMUSE(:, i)));
    
    corrSOBI(1, i) = corr((Ztrue(5,:)'),( dataSOBI(1,:))');
    if numComponentsSOBI >= 2
        corrSOBI(2, i) = corr((Ztrue(5,:))', (dataSOBI(2,:))');
    end
    if numComponentsSOBI >= 3
        corrSOBI(3, i) = corr((Ztrue(5,:))', (dataSOBI(3,:))');
    end
    if numComponentsSOBI >= 4
        corrSOBI(4, i) = corr((Ztrue(5,:))', (dataSOBI(4,:))');
    end
    if numComponentsSOBI >= 5
        corrSOBI(5, i) = corr((Ztrue(5,:))', (dataSOBI(5,:))');
    end
    best_corSOBI(5, i) = max(abs(corrSOBI(:, i)));
    ...
    corrSOBI_avg( :, n)=mean(best_corSOBI,2);
        ...
        hold on;
        plot( EbNoVec,corrfastICA_avg(4,:))
        hold on
        plot(EbNoVec,corrfastICA_avg(5,:))
        legend('T is 3','T is 4','T is 5','T is 6','T is 7')
        grid on;
        xlabel('snr')
        ylabel('correlation')
        title('correlation using ICA algorithm fastICA on Sinusoid signals');
        ...
        hold on;
        plot( EbNoVec,corrAMUSE_avg(4,:))
        hold on
        plot(EbNoVec,corrAMUSE_avg(5,:))
        legend('T is 3','T is 4','T is 5','T is 6','T is 7')
        grid on;
        xlabel('snr')
        ylabel('correlation')
        title('correlation using ICA algorithm AMUSE on Sinusoid signals');
        ...
        hold on;
        plot( EbNoVec,corrSOBI_avg(4,:))
        hold on
        plot(EbNoVec,corrSOBI_avg(5,:))
        legend('T is 3','T is 4','T is 5','T is 6','T is 7')
        grid on;
        xlabel('snr')
        ylabel('correlation')
        title('correlation using ICA algorithm SOBI on Sinusoid signals');
